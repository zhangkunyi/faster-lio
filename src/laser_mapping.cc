    ...
    if (pcd_save_en_) {
        *pcl_wait_save_ += *laserCloudWorld;
        ...
        if (pcl_wait_save_->size() > 0 && pcd_save_interval_ > 0 && scan_wait_num >= pcd_save_interval_) {
            pcd_index_++;
            if (!std::filesystem::exists(std::string(std::string(ROOT_DIR) + "PCD")))
                std::filesystem::create_directory(std::string(std::string(ROOT_DIR) + "PCD"));
...
void LaserMapping::Savetrajectory(const std::string &traj_dir) {
    if (!std::filesystem::exists(traj_dir)) std::filesystem::create_directory(traj_dir);

    std::ofstream ofs;
    ofs.open(traj_dir + "/traj.txt", std::ios::out);
    if (!ofs.is_open()) {
        LOG(ERROR) << "Failed to open traj_file: " << traj_dir + "/traj.txt";
        return;
    }
